...
configurations = [
    ...
    ['custom_heuristic_1', 'custom_heuristic_2', 'AlphaBeta', 'AlphaBeta', 2, 2, 4, np.inf],

    # ESPERIMENTO 3
    # AlphaBeta-AlphaBeta con differenti euristiche e TIMEOUT
    ...
    ['custom_heuristic_0', 'custom_heuristic_0', 'AlphaBeta', 'AlphaBeta', 2, 1, 1, np.inf],
    ['custom_heuristic_0', 'custom_heuristic_0', 'AlphaBeta', 'AlphaBeta', 3, 1, 1, np.inf],
    ['custom_heuristic_0', 'custom_heuristic_0', 'AlphaBeta', 'AlphaBeta', 2, 2, 1, np.inf],  # qua si dovrebbe riottenere un 50% - 50% 
...
with open('results.csv', 'w') as f:
    print('white_heuristic; black_heuristic; white_algorithm; black_algorithm; white_depth; black_depth; granularity; timeout [s]; white_wins [matches]; black_wins[matches]; stalemates [matches]; mean_match_time [s]; mean_turns_number', file=f)
...
for configuration in configurations:
    ...
    with open('results.csv', 'a') as f:
        print('Results:\n', '\n\twhite_wins: ', white_wins, 'matches', '\n\tblack_wins: ', black_wins, 'matches', '\n\tstalemates: ', stalemates, 'matches', '\n\tMean_execution_time: ', mean_time[-1], 's', '\n\tMean_number_of_turns: ', mean_number_of_turns[-1],'\n')
        timeout = configuration[7] if configuration[7] != np.inf else 'inf'
        print(configuration[0], configuration[1], configuration[2], configuration[3], configuration[4], configuration[5], configuration[6], timeout, white_wins, black_wins, stalemates, mean_time[-1], mean_number_of_turns[-1], sep='; ', file=f)
